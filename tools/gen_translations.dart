...
void main() async {
  ...
  if (!jmdictFile.existsSync() || !kanjidic2File.existsSync()) {
    stderr.writeln(
      'Missing input files. See the instructions at the top of this script.',
    );
    exit(1);
  }
  ...
  final jmdict =
      jsonDecode(await jmdictFile.readAsString()) as Map<String, dynamic>;
  ...
  for (final word in jmdictWords) {
    final kanjiForms = (word['kanji'] as List<dynamic>)
        .map((k) => k['text'] as String)
        .toList();
    final readingForms = (word['kana'] as List<dynamic>)
  ...
  for (final slug in ['n5', 'n4', 'n3', 'n2', 'n1']) {
    ...
    stdout.writeln(
      'kanji_$slug.json: enriched $kanjiHits / ${kanjiList.length} entries',
    );
    ...
    stdout.writeln(
      'vocab_$slug.json: enriched $vocabHits / ${vocabList.length} entries',
    );
  }
