...
class KanjiDrawingCanvas extends StatefulWidget {
  ...
  final bool enabled;
  final bool ghostEnabled; // show reference as faint overlay
  final bool snapToReference; // replace correct strokes with clean ref path
  final List<ui.Path>? hintStrokes; // highlighted hint paths drawn on canvas
...
class KanjiDrawingCanvasState extends State<KanjiDrawingCanvas> {
  ...
  Widget build(BuildContext context) {
    ...
    final painter = _CanvasPainter(
      ...
      ghostEnabled: widget.ghostEnabled,
      snapToReference: widget.snapToReference,
      hintStrokes: widget.hintStrokes,
...
class _CanvasPainter extends CustomPainter {
  ...
  final List<ui.Path>? hintStrokes;
  ...
  const _CanvasPainter({
    ...
    required this.snapToReference,
    required this.hintStrokes,
    ...
    required this.wrongStrokeOpacity,
    required this.pendingWrong,
  });
  ...
  void _drawReferenceGhost(Canvas canvas, Size size) {
    if (referenceStrokes == null || !ghostEnabled) return;
    final paint = Paint()
  ...
  void _drawHintStrokes(Canvas canvas, Size size) {
    if (hintStrokes == null || hintStrokes!.isEmpty) return;
    final paint = Paint()
      ..color = hintStrokeColor.withValues(alpha: 0.65)
      ..style = PaintingStyle.stroke
      ..strokeWidth = 5.0
      ..strokeCap = StrokeCap.round
      ..strokeJoin = StrokeJoin.round;
  ...
  void _drawStrokes(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < strokes.length; i++) {
      final bool? result = strokeResults != null && i < strokeResults!.length
          ? strokeResults![i]
          : null;
      final isPendingWrongStroke =
          result == null && pendingWrong && i == strokes.length - 1;
      final color = isPendingWrongStroke
          ? wrongColor.withValues(alpha: wrongStrokeOpacity)
          : result == null
          ? strokeColor
          : result
          ? correctColor
      ...
      if (result == true &&
          ...
          i < referenceStrokes!.length) {
        // Replace user stroke with clean reference path
        canvas.save();
        canvas.scale(size.width / kanjiVgViewBox, size.height / kanjiVgViewBox);
        canvas.drawPath(referenceStrokes![i], basePaint..color = correctColor);
        canvas.restore();
      } else {
