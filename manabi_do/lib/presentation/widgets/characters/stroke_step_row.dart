...
class _StrokeStepRowState extends ConsumerState<StrokeStepRow> {
  ...
  void _onDragEnd(DragEndDetails details) {
    ...
    final pos = _scrollController.position;
    final target =
        (_scrollController.offset - details.velocity.pixelsPerSecond.dx * 0.12)
  ...
  Widget build(BuildContext context) {
    final strokes = ref
        .watch(kanjiStrokesProvider(widget.kanjiId))
        .asData
        ?.value;
...
class _StepPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= step && i < strokes.length; i++) {
      canvas.drawPath(
        strokes[i],
        base..color = (i < step ? prevColor : currColor),
      );
    }
  }
